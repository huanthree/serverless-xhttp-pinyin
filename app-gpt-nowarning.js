/**
 * 修正说明：
 * 1. 在最开头增加了对 DeprecationWarning 的监听，只忽略 code 为 'DEP0180' 的警告。
 * 2. 其余部分沿用了原始代码逻辑，没有删除任何业务功能。
 * 3. 这样做之后，程序启动时即使底层有模块调用了 new fs.Stats()，也不会再向控制台打印弃用警告，也不会影响流程。
 */

'use strict'; // 推荐开启严格模式

// —— （1）屏蔽 fs.Stats constructor 弃用警告 —— //
process.on('warning', (warning) => {
    // 如果是 fs.Stats constructor 弃用警告（code: DEP0180），就忽略掉
...
const UUID = process.env.UUID || '796d0aaf-77e6-4588-8c65-7b8a1ebdd358';
const XPATH = process.env.XPATH || UUID.slice(0, 8);
const SUB_PATH = process.env.SUB_PATH || 'sub';
const DOMAIN = process.env.DOMAIN || 'Node2.LynxNodes.ge';
const NAME = process.env.NAME || 'huan-LynxNodes.es';
const PORT = process.env.PORT || 25658;
...
class Session {
    ...
    async initializeVLESS(firstPacket) {
        ...
        try {
            ...
            log(
                'info',
                `VLESS header parsed: ${this.vlessHeader.hostname}:${this.vlessHeader.port}`
            );
            this.remote = await connect_remote(
                this.vlessHeader.hostname,
                this.vlessHeader.port
            );
            ...
            while (this.pendingBuffers.has(this.nextSeq)) {
                ...
                if (!this.initialized && this.nextSeq === 0) {
                    ...
                    if (!this.initialized) {
                        log(
                            'warn',
                            `Received out of order packet seq=${seq} before initialization`
                        );
                        continue;
                    }
    ...
    _startDownstreamResponse() {
        ...
        try {
            const protocol =
                this.currentStreamRes.socket?.alpnProtocol || 'http/1.1';
            const isH2 = protocol === 'h2';
            if (!this.headerSent) {
                log(
                    'debug',
                    `Sending VLESS response header (${protocol}): ${
                        this.responseHeader.length
                    } bytes`
                );
...
if (!DOMAIN) {
    try {
        IP = execSync('curl -s --max-time 2 ipv4.ip.sb', {
            encoding: 'utf-8',
        }).trim();
    } catch (err) {
        try {
            IP = `[${execSync('curl -s --max-time 1 ipv6.ip.sb', {
                encoding: 'utf-8',
            }).trim()}]`;
        } catch (ipv6Err) {
        ...
        if (!session) {
            ...
            setTimeout(() => {
                ...
                if (currentSession && !currentSession.downstreamStarted) {
                    log(
                        'warn',
                        `Session ${uuid} timed out without downstream`
                    );
...
server.on('secureConnection', (socket) => {
    log(
        'debug',
        `New secure connection using: ${socket.alpnProtocol || 'http/1.1'}`
    );
});
